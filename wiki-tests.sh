...
if [ "$4" == "update" ]; then
    ...
    wget -O $TMPHTML -q http://wiki.apertium.org/wiki/Iranian_Persian_and_Tajik/$TESTTYPE
    if [[ -s $TMPHTML ]]; then mv $TMPHTML $HTML;
    else rm $TMPHTML; echo "Couldn't fetch http://wiki.apertium.org/wiki/Iranian_Persian_and_Tajik/$TESTTYPE"; fi
fi
echo "..."
